...
public class ExpandBorderCommand implements CommandExecutor {
    @Override
    public boolean onCommand( CommandSender sender,  Command command,  String label,  String[] args) {
        if (sender instanceof Player) {

            if(args.length>2||args.length<1){
                sender.sendMessage(MiniMessage.miniMessage().deserialize(EventCore.red+"Error, usage:/expand <distance> [delay]"));
                return true;
            }
                ...
                if (Bukkit.getWorld("world") != null) {
                    ...
                    for (Player p : Bukkit.getOnlinePlayers()) {
                            p.sendMessage(MiniMessage.miniMessage().deserialize(EventCore.green+"<bold>Border is now expanding! ["+df.format((double)distance/2/time)+" blocks/s]"));
                        }
